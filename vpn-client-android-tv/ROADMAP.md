...
- [x] Health Monitor (active probe + passive + wake detection, 3-7 сек, пороги 20с/30с)
...
### ✅ Этап 13: Надёжный мониторинг соединения (v2.0.2)
- [x] Active Probe — фиксация `lastKeepaliveSent`, обнаружение мёртвого сервера за 15 сек (вместо 60 сек)
- [x] Wake Detection — wall clock vs monotonic, мгновенный reconnect после сна устройства
- [x] ConnectivityManager — системный callback `onLost()`/`onAvailable()` → немедленный `forceReconnect()`
- [x] Снижение порогов: Degraded 35→20 сек, Lost 60→30 сек
- [x] Метод `forceReconnect()` — принудительное закрытие сокета из внешнего контекста
