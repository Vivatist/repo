...
go build -trimpath -ldflags="-s -w -H windowsgui" -o NovaVPN.exe ./cmd/novavpn/
if errorlevel 1 (
...
go build -trimpath -ldflags="-s -w" -o novavpn-service.exe ./cmd/novavpn-service/
if errorlevel 1 (
...
:: Копируем файлы
echo   [→] Копирование файлов в dist\NovaVPN\...
copy /Y NovaVPN.exe dist\NovaVPN\ >nul
copy /Y novavpn-service.exe dist\NovaVPN\ >nul
