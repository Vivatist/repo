...
каждые 15 секунд (в keepalive-цикле):
    ...
    если elapsed > 20 секунд:   // ~1 пропущенный keepalive + запас
        логировать "Dead peer detected"
        закрыть UDP-соединение
        → автоматический reconnect
```

**Дополнительно:** клиент запускает `networkMonitorLoop` — каждые 2 секунды опрашивает состояние физических сетевых интерфейсов. При исчезновении всех физических IPv4-адресов немедленно закрывает UDP-соединение и запускает reconnect, не дожидаясь DPD.
...
| Dead-peer порог | 20 сек | ~1 пропущенный keepalive + запас |
