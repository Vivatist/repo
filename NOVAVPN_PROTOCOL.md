...
> **Active Probe**: клиент фиксирует время отправки каждого keepalive (`lastKeepaliveSent`). Если за 15 секунд после отправки не пришёл ни один пакет от сервера — health monitor определяет соединение как мёртвое и запускает reconnect. Это не генерирует дополнительного трафика — используются существующие keepalive.
...
10. Запустить 4 горутины/потока:
      ...
      - Health Monitor (active probe + wake detection + passive, 3-7 сек, пороги 20с/30с, dead-peer detection)
